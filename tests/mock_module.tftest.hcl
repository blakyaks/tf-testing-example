...
# Mock providers will be used instead of real provider instances in this test
mock_provider "local" {}

mock_provider "external" {

  # Override the data returned by the external provider data source
  # In this case we simulate the presence of the environment variable and the expected value
  mock_data "external" {
    defaults = {
      result = {
        USER_VAR = "1"
